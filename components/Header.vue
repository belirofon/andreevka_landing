    ...
    <div class="container mx-auto px-4 flex justify-between items-center">
      <div class="flex items-center">
        <h1 class="text-xl md:text-2xl font-bold">
          <span :class="scrolled ? 'text-primary-700' : 'text-white'">Андреевка</span>
        </h1>
      </div>
      ...
      <nav class="hidden md:flex space-x-8">
        <a v-for="item in menuItems" :key="item.href" :href="item.href"
          :class="[
            ...
            scrolled ? 'text-secondary-700 hover:text-primary-600' : 'text-white hover:text-primary-100'
          ]">
          {{ item.label }}
        </a>
      </nav>
      ...
      <button @click="isMenuOpen = !isMenuOpen" class="md:hidden" 
        :class="scrolled ? 'text-secondary-800' : 'text-white'">
